...
Partial Class Admin_ReceiptGenerate
    Inherits BasePage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            Try
                ...
                If Request.QueryString("receipt") Is Nothing Then
                    txtReceiptno.Text = BLL.ExecScalar("Select dbo.GetNewSessionReceiptNo(@SchoolSession)", "@SchoolSession", ddlSession.SelectedValue)
                ...
                Else
                    Dim receiptcnt As Integer = Request.QueryString("receipt")
                    txtReceiptno.Text = BLL.ExecScalar("Select SessionReceiptNo from receiptmaster Where Cnt=@ReceiptCnt", "@ReceiptCnt", receiptcnt).ToString()
                    ddlSession.SelectedValue = BLL.ExecScalar("Select SchoolSession from receiptmaster Where Cnt=@ReceiptCnt", "@ReceiptCnt", receiptcnt).ToString()
                    txtReceiptnoPaid.Text = txtReceiptno.Text
                    Dim todaysdate As String = String.Format("{0:dd/MM/yyyy}", DateTime.Now)
                    txtDueDate.Text = todaysdate
                    txtDueDatePaid.Text = todaysdate
                    If Not Session("ReceiptDate") Is Nothing Then
                        txtDueDate.Text = Session("ReceiptDate")
                    End If
                    Dim StudentId As String = BLL.ExecScalar("Select StudentId from receiptmaster Where Cnt=@receiptcnt", "@receiptcnt", receiptcnt)
                    ...
                    hfId.Value = 0
                    CreateTable()
                    bindReceipt(receiptcnt)
                    BindStudentData()
                    bindPDCBilling()
                    bindFuturePDC()
                    BuildDataDisplay()
                    ddlsearch.SelectedValue = StudentId
                    activateCashChequeDiv()
                    BindBankName()
    ...
    Sub bindPDCBilling()
        ...
        End If
    End Sub
    Sub bindReceipt(ByVal ReceiptNoCnt As Integer)
        Dim dt As DataTable = New DataTable()
        If txtStudentId.Text <> "" Then
            Dim workTable As DataTable = DirectCast(ViewState("tblFee"), DataTable)
            dt = BLL.ExecDataTableProc("Get_Receiptdetails", "@ReceiptNoCnt", ReceiptNoCnt)
    ...
    Protected Sub btnSubmit_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnSubmit.Click
        Try
            ...
            If dt.Rows.Count > 0 Then
                res = BLL.AddReceiptSession(txtStudentId.Text, txtReceiptno.Text, Val(txtReceiptAmt.Text), Val(txtDiscount.Text), Val(txtlatefeesPaid.Text), txtDueDate.Text, Session("Operator"), ddlDiscount.SelectedItem.Text.ToString(), ddlDiscount.SelectedValue, dt, ddlSession.SelectedValue)
                If res.Chars(0) = "#" Then
    ...
    Protected Sub DataDisplay_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles DataDisplay.RowCommand
        If e.CommandName = "Insert" Then
            ...
            Dim txtnewFeeType As TextBox = DirectCast(DataDisplay.FooterRow.FindControl("txtnewFeeType"), TextBox)
            ...
            If Val(txtnewFeeType.Text <> "" And newamount > 0) Then
                hfId.Value = hfId.Value + 1
                ...
                'End If
                workTable.Rows.Add(New Object() {hfId.Value, txtnewFeeType.Text, 0, Val(txtnewAmount.Text), newLateFee, 0})
                ViewState("tblFee") = workTable
                litmsg.Text = ""
            Else
    ...
    Protected Sub txtStudentId_TextChanged(sender As Object, e As System.EventArgs) Handles txtStudentId.TextChanged
        Dim dr As SqlDataReader = BLL.ExecDataReader("Select s.StudentId,s.StudentName,s.MainClassId,c.MainClassName from StudentMaster S left join Mainclassmaster c on s.MainClassId=c.MainClassId Where s.StudentId=@StudentId and c.MainClassId>0", "@StudentId", txtStudentId.Text)
        If dr.Read() Then
